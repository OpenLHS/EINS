...
# PROJECT: Atelier d'introduction à R (Exercices)
# DOC:     
# BY:      JPM, FCL
# DATE:    Mai 2025
# UPDATE:  Mai 2026  
...
# Évidemment, rien ne nous empêche d'utiliser d'autres ressources pour mieux comprendre comment utiliser une fonction
# ou encore pour comprendre comment elle fonctionne. Les LLM (ChatGPT, Claude, Gemini, Copilot, etc) sont plutôt bons
...
# Note: Comme il n'est pas nécessaire d'installer plusieurs fois une même librairies, je vous conseille de commenter
...
# Pré-requis: la gestion des répertoires
# Pour voir le contenu du répertoire dans lequel on travail présentement.
dir()
...
# Les principaux verbes à notre disposition sont les suivants:
# filter()
# select()
# mutate()
# arrange()
...
# Seconde comparaison avec l'opérateur %>% 
df_glucose_sans_traitement <- filter(df_glucose, Traitement==0)
mean(df_glucose_sans_traitement$IMC)
df_glucose_avec_traitement <- filter(df_glucose, Traitement==1)
mean(df_glucose_avec_traitement$IMC)
...
# On aimerait connaitre l'age moyen des patientes qui ont reçu le traitement, selon
# si elles font parti d'un groupe à risque ou non.
...

# Pour les patientes diagnostiquées avec un prédiabète et qui sont agés de plus de 30 ans,
# combien d'entre elles font parti d'un groupe à risque?
...
...
# Supposons que l'on souhaite crée un histogramme de l'age des patientes
# Nous allons créer le graphiques couche par couche
...
# On peut également spécifier certains paramètres pour le représentation géométrique.
...
# Enfin, on peut également faire ce qu'on appelle un choix de facettes.
ggplot(df_glucose, aes(x=Age)) + 
  geom_histogram(binwidth = 1) + 
...
# Traitement est une variable numérique, mais l'argument fill devrait être catégorique. On crée donc une autre
...
# Pour ce même nuage de points, on aimerait colorier les points afin de différencier les patientes faisant parti d'un
# groupe à risque et celles qui n'en font pas parti
# (Indication: Il est recommandé d'utiliser une version catégorique de la variable "Groupe à risque"!)
...
...
# les patientes faisant parti d'un groupe à risque.
...
...
# Que se passe-t-il si on déplace l'argument concernant la couleur dans la fonction geom_point() ?
...
...
# Plusieurs fonctions en R sont disponibles et permettent de calculer les statistiques 
# descriptives pour une série de données. Les fonctions suivantes pourraient donc
# vous être utiles:
#   mean()
#   min()
#   max()
...
# et du groupe "controle" pour les variables suiavntes:
...
# du test oral inferieur a 0.3mmol/L
seuil = 0.3 
...
df_glucose %>% 
  ...
  count(Amelioration) %>% 
  pivot_wider(names_from = TraitementCat, values_from = n) %>% 
  column_to_rownames("Amelioration") %>% 
  as.matrix() %>% 
  prop.test()
...
# On crée la variable categorique pour l'age, si ce n'est pas déjà fait
df_glucose <- df_glucose %>% 
...
# Ce n'est pas nécessaire, mais ces valeurs s'interprête beaucoup plus facilement.
